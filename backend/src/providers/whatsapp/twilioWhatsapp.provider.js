...
const withWhatsappPrefix = (n) => (n?.startsWith('whatsapp:') ? n : `whatsapp:${n}`);
...
const post = async (config, params) => {
...
exports.send = async (config, { mobile, message }) => {
  ...
  try {
    const { ok, status, data } = await post(config, new URLSearchParams({
      To: withWhatsappPrefix(to),
      From: withWhatsappPrefix(config.fromNumber),
      Body: message
    }));
    if (!ok) return { success: false, error: data.message || data.raw || `Twilio request failed (${status})`, providerResponse: data };
...
exports.sendDocument = async (config, { mobile, documentUrl, filename }) => {
  const to = normalizeE164(mobile);
  if (!to) return { success: false, error: 'No mobile number' };
