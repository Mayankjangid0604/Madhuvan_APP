          ...
          {filtered.map(student => {
            ...
            return (
              <div key={`student-${student.student_id}`} className={`student-card status-${status.color}`}>
                ...
                {/* ── Only ONE Pay All Remaining button ── */}
                {student.total_remaining > 0 && (
                  <div className="card-pay-section">
                    <button
                      className="pay-btn pay-btn-all"
                      style={{ width: '100%', background: 'linear-gradient(135deg, #7c3aed, #4f46e5)', color: '#fff', border: 'none', borderRadius: '10px', padding: '12px 16px', fontWeight: 700, fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                      onClick={(e) => { e.stopPropagation(); openPayModal(student, "ALL"); }}
                    >
                      <CreditCard size={16} />
                      <span>Pay All Remaining {formatCurrency(student.total_remaining)}</span>
                    </button>
                  </div>
                )}
